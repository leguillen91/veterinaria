...
export const mostrarEmpleados = async (req, res) => {
  const [rows] = await pool.query("SELECT * FROM persona p JOIN empleado e on p.personaID = e.personaID");
...
export const crearEmpleado = async (req, res) => {
  ...
  try {
    ...
    await pool.query("INSERT INTO empleado SET personaID = ?, cargoID = ? ", [personaID, newCustomer.cargo]);
...
export const editEmpleados = async (req, res) => {
  const { id } = req.params;
  const [result] = await pool.query("SELECT * FROM personas p JOIN empleados e on p.identidad = e.identidad p.id = ?", [
    id,
  ]);
...
export const actualizarEEmpleados = async (req, res) => {
  const { id } = req.params;
  const newCustomer = req.body;
  await pool.query("UPDATE empleados set ? WHERE id_empleado = ?", [newCustomer, id]);
  res.redirect("/");
};

export const borrarEmpleado = async (req, res) => {
  const { id } = req.params;
  const result = await pool.query("DELETE FROM personas WHERE id_empleado = ?", [id]);
  if (result.affectedRows === 1) {
